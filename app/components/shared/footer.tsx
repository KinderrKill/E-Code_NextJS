...
export default function Footer() {
  ...
  return (
    <div className='bg-third py-7 flex flex-col items-center justify-center shadow-black shadow-inner'>
      <div className='flex gap-10 my-5'>
        {socialNetwork.map((item: SocialNetwork, index: number) => (
          <a href={item.link} key={index} className='hover:scale-105 transition-all'>
            <Image src={item.icon} alt={item.alt} className='w-8 h-8 rounded-lg' />
          </a>
        ))}
      </div>
      <span>
